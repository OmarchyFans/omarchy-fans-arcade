...
# ---- this plugin -------------------------------------------------------------
UPD_ID="fans.omarchy.arcade"
UPD_NAME="Arcade"
UPD_REPO="OmarchyFans/omarchy-fans-arcade"            # GitHub owner/repo the plugin is published from
UPD_BRANCH="main"        # branch whose manifest.json is "the published version"
UPD_SLUG="omarchy-arcade"            # cache lives in ~/.cache/<slug>/update-check.json
UPD_CONFIG="${XDG_CONFIG_HOME:-$HOME/.config}/omarchy-arcade/config.json"        # JSON file; "update_check": false turns the check off
...
post_update() {
  # Arcade links nothing outside this folder: nothing else to refresh.
  :
}
# ------------------------------------------------------------------------------
